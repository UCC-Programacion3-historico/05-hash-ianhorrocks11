...
HashMap<K, T>::HashMap(unsigned int tamanio) {
        ...
        this->tamanio = tamanio; //quiero pasarlo como parametro
        hashFuncP = hashFunc;
...
void HashMap<K, T>::put(K clave, T valor) {
    unsigned int pos = hashFunc(clave) % tamanio; // ver pos
    if (tabla[pos] != NULL) // si hay algo quiere decir que no es null
        throw 1;
...
bool HashMap<K, T>::esVacio() {
    ...
    for(int i = 0; i > tamanio; i++){
        if(tabla[i] != NULL) // recorre , apenas ve un null, quiere decir que no estaba vacia
            return false;
    }
    return true;
}
